...
public class EntityAudioChannelImpl extends AudioChannelImpl implements EntityAudioChannel {
    ...
    private void broadcast(PlayerSoundPacket packet) {
        if (entity.getEntity() instanceof org.bukkit.entity.Entity entity) {
            server.broadcast(ServerWorldUtils.getPlayersInRange(entity.getWorld(), entity.getLocation(), Voicechat.SERVER_CONFIG.voiceChatDistance.get(), filter == null ? player -> true : player -> filter.test(new ServerPlayerImpl(player))), packet, null, null, null);
        }
    }
