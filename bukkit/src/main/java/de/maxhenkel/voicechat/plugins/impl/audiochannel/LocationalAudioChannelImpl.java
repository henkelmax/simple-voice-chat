...
public class LocationalAudioChannelImpl extends AudioChannelImpl implements LocationalAudioChannel {
    ...
    private void broadcast(LocationSoundPacket packet) {
        server.broadcast(ServerWorldUtils.getPlayersInRange((World) level.getServerLevel(), position.getPosition(), server.getBroadcastRange(), filter == null ? player -> true : player -> filter.test(new ServerPlayerImpl(player))), packet, null, null, null, SoundPacketEvent.SOURCE_PLUGIN);
    }
