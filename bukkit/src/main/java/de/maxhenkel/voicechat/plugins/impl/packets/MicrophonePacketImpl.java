...
public class MicrophonePacketImpl implements MicrophonePacket {
    ...
    public LocationalSoundPacket toLocationalSoundPacket(Position position, float distance) {
        if (position instanceof PositionImpl) {
            return new LocationalSoundPacketImpl(new LocationSoundPacket(sender, ((PositionImpl) position).getPosition(), packet.getData(), packet.getSequenceNumber(), distance));
        } else {
