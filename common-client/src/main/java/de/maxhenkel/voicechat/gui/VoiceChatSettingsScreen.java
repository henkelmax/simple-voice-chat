...
public class VoiceChatSettingsScreen extends VoiceChatScreenBase {
    ...
    private static final Component ASSIGN_TOOLTIP = new TranslatableComponent("message.voicechat.press_to_reassign_key");
    private static final Component PUSH_TO_TALK = new TranslatableComponent("message.voicechat.activation_type.ptt");
    ...
    protected void init() {
        ...
        MicAmplificationSlider micAmp = new MicAmplificationSlider(this, guiLeft + 10 + (agc ? 80 + 1 : 0), y, xSize - 20 - (agc ? 80 : 0) - 1, 20);
        if (agc) {
            addRenderableWidget(new AgcButton(this, guiLeft + 10, y, 80, 20, active -> micAmp.setActive(!active)));
        }
        ...
        y += 21;
        addRenderableWidget(new Button(guiLeft + 10, y, (xSize - 20) / 2 - 1, 20, SELECT_MICROPHONE, button -> {
            minecraft.setScreen(new SelectMicrophoneScreen(this));
        }));
        addRenderableWidget(new Button(guiLeft + xSize / 2 + 2, y, (xSize - 20) / 2 - 1, 20, SELECT_SPEAKER, button -> {
