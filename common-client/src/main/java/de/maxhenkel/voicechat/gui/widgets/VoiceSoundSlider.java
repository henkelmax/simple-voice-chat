...
public class VoiceSoundSlider extends DebouncedSlider {
    ...
    public VoiceSoundSlider(int x, int y, int width, int height, float maxVolume) {
        super(x, y, width, height, new StringTextComponent(""), VoicechatClient.CLIENT_CONFIG.voiceChatVolume.get().floatValue() / maxVolume);
    ...
    public ITextComponent getMsg() {
        return new TranslationTextComponent("message.voicechat.voice_chat_volume", Math.round(value * maxVolume * 100F) + "%");
    }
