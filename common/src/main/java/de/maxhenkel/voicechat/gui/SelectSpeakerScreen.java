...
public class SelectSpeakerScreen extends ListScreen<String> {
    ...
    public SelectSpeakerScreen(Screen parent) {
        super(parent, SoundManager.getAllSpeakers(), new TranslationTextComponent("gui.voicechat.select_speaker.title"));
    ...
    protected void renderText(MatrixStack stack, @Nullable String element, int mouseX, int mouseY, float partialTicks) {
        ...
        IFormattableTextComponent name = getCurrentElement() == null ? new TranslationTextComponent("message.voicechat.no_speaker") : new StringTextComponent(SoundManager.cleanDeviceName(getCurrentElement()));
