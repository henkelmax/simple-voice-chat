...
public class EntityAudioChannelImpl extends AudioChannelImpl implements EntityAudioChannel {
    ...
    private void broadcast(PlayerSoundPacket packet) {
        if (entity.getEntity() instanceof net.minecraft.world.entity.Entity entity) {
            server.broadcast(ServerWorldUtils.getPlayersInRange((ServerLevel) entity.level, entity.getEyePosition(), server.getBroadcastRange(), filter == null ? player -> true : player -> filter.test(new ServerPlayerImpl(player))), packet, null, null, null, SoundPacketEvent.SOURCE_PLUGIN);
        }
    }
