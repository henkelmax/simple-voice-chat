...
public class Server extends Thread {
    ...
    private void processMicPacket(ServerPlayerEntity player, PlayerState state, MicPacket packet) throws Exception {
        if (state.hasGroup()) {
    ...
    private void processProximityPacket(PlayerState senderState, ServerPlayerEntity sender, MicPacket packet) throws Exception {
        ...
        if (sender.isSpectator()) {
            ...
            if (Voicechat.SERVER_CONFIG.spectatorInteraction.get()) {
                soundPacket = new LocationSoundPacket(sender.getUUID(), sender.getUUID(), sender.getEyePosition(1F), packet.getData(), packet.getSequenceNumber(), distance, null);
    ...
    public void sendSoundPacket(@Nullable ServerPlayerEntity sender, @Nullable PlayerState senderState, ServerPlayerEntity receiver, PlayerState receiverState, @Nullable ClientConnection connection, SoundPacket<?> soundPacket, String source) throws Exception {
        ...
        if (!PermissionManager.INSTANCE.LISTEN_PERMISSION.hasPermission(receiver)) {
            CooldownTimer.run(String.format("no-listen-%s", receiver.getUUID()), 30_000L, () -> {
                receiver.displayClientMessage(new TranslationTextComponent("message.voicechat.no_listen_permission"), true);
            });
            return;
        }
