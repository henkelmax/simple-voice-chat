...
public class MicrophonePacketImpl implements MicrophonePacket {
    ...
    public LocationalSoundPacket toLocationalSoundPacket(Position position) {
        return new LocationalSoundPacketImpl(new LocationSoundPacket(sender, sender, position, packet.getData(), packet.getSequenceNumber(), Utils.getDefaultDistanceServer(), null));
    }
