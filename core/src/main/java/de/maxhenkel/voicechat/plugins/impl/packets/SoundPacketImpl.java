...
public class SoundPacketImpl implements SoundPacket {
    ...
    public LocationalSoundPacket toLocationalSoundPacket(Position position) {
        return new LocationalSoundPacketImpl(new LocationSoundPacket(packet.getChannelId(), packet.getSender(), position, packet.getData(), packet.getSequenceNumber(), getDistance(), null));
    }
