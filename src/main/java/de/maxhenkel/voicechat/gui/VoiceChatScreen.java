...
public class VoiceChatScreen extends Screen implements MicTestButton.MicListener {
    ...
    protected void init() {
        ...
        addButton(new Button(guiLeft + 10, guiTop + 170, xSize - 20, 20, new TranslationTextComponent("message.adjust_volumes").getString(), button -> {
            Main.SIMPLE_CHANNEL.sendToServer(new RequestPlayerListMessage());
        }));
        addButton(new Button(guiLeft + 10, guiTop + 195, xSize / 2 - 15, 20, new TranslationTextComponent("message.select_microphone").getString(), button -> {
            minecraft.displayGuiScreen(new SelectMicrophoneScreen());
        }));
        addButton(new Button(guiLeft + xSize / 2 + 6, guiTop + 195, xSize / 2 - 15, 20, new TranslationTextComponent("message.select_speaker").getString(), button -> {
